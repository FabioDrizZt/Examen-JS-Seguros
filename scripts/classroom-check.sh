#!/usr/bin/env bash
# Verificaciones automáticas — Examen JS Seguros.
set -u
...
case "${1:-}" in
  ...
  fetch-api)
    ...
    echo "$js_code" | grep -qiE '(/api/propiedades|/api/ubicaciones|/api/seguros|localhost:3000)' \
      || fail "No se encontró la consulta a las rutas del backend (/api/propiedades, /api/ubicaciones o /api/seguros) en js/script.js."
    ok
    ;;
  render-dom)
    [[ -f "$JS" ]] || fail "No se encontró el archivo js/script.js."
    js_code="$(clean_js)"
    echo "$js_code" | grep -qiE '(propiedad|ubicacion|metros2|valorPoliza|historialLista)' \
      || fail "No se encontró la selección de los elementos del DOM (propiedad, ubicacion, metros2, valorPoliza)."
    if echo "$js_code" | grep -qiE '(innerHTML|appendChild|createElement|textContent|insertAdjacentHTML)'; then
      :
    else
      fail "No se encontró manipulación del DOM para renderizar las opciones de propiedad, ubicación o el resultado."
    fi
    ok
    ;;
  events-logic)
    ...
    echo "$js_code" | grep -qiE '(factor|poliza|valor|metros|costo|total)' \
      || fail "No se encontró la lógica de cálculo del costo de la póliza de seguro."
    ok
    ;;
  storage-history)
    [[ -f "$JS" ]] || fail "No se encontró el archivo js/script.js."
    ...
    echo "$js_code" | grep -qiE 'localStorage\s*\.\s*setItem\s*\(' \
      || fail "No se encontró el uso de localStorage.setItem() para persistir las cotizaciones de seguros."
    echo "$js_code" | grep -qiE 'localStorage\s*\.\s*getItem\s*\(' \
      || fail "No se encontró el uso de localStorage.getItem() para recuperar las cotizaciones de seguros."
